...
-- SIMULAR TOQUE EN EL BUZÓN (CORREGIDO)
local function SimulateMailboxTouch()
    ...
    -- Método 3: Teletransporte rápido
...
--// 1. AUTO RECOGER DINERO (CORREGIDO)
        ...
        ProcessManager:Start(processId, function()
            ...
            for _, button in pairs(allButtons) do
                if button.Visible and button.Active then
                    local name = string.lower(button.Name or "")
                    local text = string.lower(button.Text or "")
                    if name:find("vip") or text:find("vip") or text:find("premium") or name:find("premium") then
                        pcall(function()
                            button:MouseButton1Click()
                        end)
                    end
...
print("✅ Script Cargado - Mansion Tycoon v1.0")
