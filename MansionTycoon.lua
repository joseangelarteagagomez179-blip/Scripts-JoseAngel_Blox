...
-- ========== ESPERAR QUE EL PERSONAJE EXISTA ==========
local character = player.Character or player.CharacterAdded:Wait()
local humanoid = character:WaitForChild("Humanoid")
local rootPart = character:WaitForChild("HumanoidRootPart")

-- ========== LIMPIAR GUÍAS VIEJAS (CON PCALL) ==========
pcall(function()
    for _, child in pairs(player:WaitForChild("PlayerGui"):GetChildren()) do
        if child.Name == "MansionTycoonGUI" or child.Name == "Load_JA" then
            child:Destroy()
        end
    end
end)
...
-- 🔥 CORRECCIÓN: usar pcall para que no falle si gethui no existe
local success, parentGui = pcall(function()
    return gethui and gethui() or player.PlayerGui
end)
ScreenGui.Parent = parentGui or player.PlayerGui
...
-- ========== FILTRO MEJORADO ANTI-ROBUX ==========
local function isSafeToTouch(v)
    if not (v:IsA("BasePart") and v.Name == "Touch") then
        return false
    end
        ...
        if child:IsA("BillboardGui") or child:IsA("SurfaceGui") then
            ...
            for _, t in pairs(textElements) do
                if t:IsA("TextLabel") then
                    ...
                    if txt:find("robux") or txt:find("r%$") or txt:find("premium") or 
                       txt:find("vip") or txt:find("auto%-recoger") or 
                       txt:find("starter") or txt:find("pack") or 
                       txt:find("velocidad") or txt:find("extra") or 
                       txt:find("correo") or txt:find("doble") or 
                       txt:find("dinero") then
                        return false
                    end
                end
    ...
    -- 4. DETECTAR PRECIO DE ROBUX EN EL TEXTO DEL BOTÓN
    local allDescendants = parent:GetDescendants()
    for _, d in pairs(allDescendants) do
        if d:IsA("TextLabel") or d:IsA("TextButton") then
            local txt = d.Text:lower()
            if txt:find("r%$") or txt:find("robux") or txt:find("premium") or 
               txt:find("vip") or txt:find("starter") then
                return false
            end
        end
    end
...
createToggle(tabContents["Player"], "⚡ Speed Hack", function(state)
    humanoid.WalkSpeed = state and 60 or 16
end)
createToggle(tabContents["Player"], "🦘 Salto Alto", function(state)
...
task.spawn(function()
    ...
    for i = 1, #t1 do
        WelcomeLabel.Text = t1:sub(1, i)
        task.wait(0.05)
    end
    task.wait(0.2)
    local t2 = "Scripts JoseAngel_Blox"
    for i = 1, #t2 do
        NameLabel.Text = t2:sub(1, i)
        task.wait(0.06)
    end
    task.wait(1.5)
    LoadGui:Destroy()
