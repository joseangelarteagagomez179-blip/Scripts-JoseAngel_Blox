...
--// FUNCIONES
Btn("⚡ ACTIVAR TODO",Color3.fromRGB(70,200,255),55,function()
...
Btn("🏆 AUTO WIN",Color3.fromRGB(255,160,60),105,function()
    spawn(function() while true do task.wait(0.1) local hrp = Player.Character.HumanoidRootPart hrp.CFrame = hrp.CFrame * CFrame.new(0,0,-8) end end)
end)

Btn("🚀 IR A LA META",Color3.fromRGB(140,120,255),155,function()
    local hrp = Player.Character.HumanoidRootPart
    hrp.CFrame = CFrame.new(hrp.Position.X, hrp.Position.Y+50, hrp.Position.Z-99999)
end)

Btn("💎 COMPRAR TODO GRATIS",Color3.fromRGB(255,80,150),205,function()
    pcall(function()
        -- Dinero Infinito
        for _,v in pairs(Player.leaderstats:GetChildren()) do if v:IsA("NumberValue") or v:IsA("IntValue") then v.Value = math.huge end end
        -- Comprar Todo
        for _,v in pairs(workspace:GetDescendants()) do if v:IsA("ClickDetector") then fireclickdetector(v) task.wait(0.05) end end
    end)
end)
...
Btn("💨 VELOCIDAD MAX",Color3.fromRGB(60,255,150),305,function()
    Player.Character.Humanoid.WalkSpeed = 99999
end)

--// MINIMIZAR
Min.MouseButton1Click:Connect(function() Main.Visible = not Main.Visible end)

print("✅ DOLA HUB LISTO!")
